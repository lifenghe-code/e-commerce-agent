...
- **向量数据库**（如 Postgresql ）：存储电商领域知识（商品库、运营策略、用户历史数据），支持语义检索与快速匹配。
...
The Extract, Transform, and Load (ETL) framework。提取、转换和加载 (ETL) 框架是检索增强生成 (RAG) 用例中数据处理的支柱。
...
![image-20250609113059772](H:\Java\Project\e-commerce-agent\image-20250609113059772.png)
...
​       用AI模型将文字转化为数学向量，使语义相近的内容产生相似数学特征。

- **相似检索 → 智能资料猎人**

  将用户问题转为”问题向量”，通过多维度匹配策略搜索知识库。

- **生成增强 → 专业报告撰写**

  将检索结果作为指定参考资料，AI生成时自动关联相关知识片段。
